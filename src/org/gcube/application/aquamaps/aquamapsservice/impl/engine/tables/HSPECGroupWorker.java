	...
	public void run() {
		...
		try{
			...
			if(request.getLogic().equals(LogicType.HCAF))
				for(Field f:request.getParameters()) 
	...
	private static void simpleExecution(final AlgorithmType algorithmType,final SourceGenerationRequest theRequest, final List<Resource> sourcesSubSet,BatchGeneratorI batch,final HSPECGroupWorker worker)throws Exception{
		...
		//********** START PROCESS INIT
			...
			try{
				...
				//************* CHECK EXISTING
				Set<Resource> existing=getExisting(algorithmType, sourcesSubSet,theRequest.getId(),theRequest.getLogic(),theRequest.getParameters());
				...
				if(existing.size()==0){
					...
					batch.generateTable(
							new TableGenerationConfiguration(
									...
									theRequest.getAuthor(),theRequest.getParameters(),worker){
										...
										public void registerGeneratedSourcesCallback(
											...
											TableGenerationExecutionManager.notifyGeneration(
													new Execution(this.getAlgorithm(), theRequest.getLogic(), sourcesIds,theRequest.getField(SourceGenerationRequestFields.additionalparameters).getValue()));
