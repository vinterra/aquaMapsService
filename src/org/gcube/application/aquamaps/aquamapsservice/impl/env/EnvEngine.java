...
public abstract class EnvEngine {
	...
	protected void fillData(List<Cell> goodCells, String fld) throws Exception {
		...
		//get percentiles
		this.min = Double.parseDouble(goodCells.get(0).getFieldbyName(fld).getValue());
		this.max = Double.parseDouble(goodCells.get(reccount-1).getFieldbyName(fld).getValue());
		this.para25 = Double.parseDouble(goodCells.get((int) this.rec25).getFieldbyName(fld).getValue());
		this.para75 = Double.parseDouble(goodCells.get((int) rec75).getFieldbyName(fld).getValue());
		this.pMin = Double.parseDouble(goodCells.get((int) rec10).getFieldbyName(fld).getValue());
		this.pMax = Double.parseDouble(goodCells.get((int) rec90).getFieldbyName(fld).getValue());
