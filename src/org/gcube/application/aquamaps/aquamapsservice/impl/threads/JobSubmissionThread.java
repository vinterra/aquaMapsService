			...
			for(AquaMapsObject aquaMapObj:toPerform.getAquaMapsObjectList()){
				...
				if((aquaMapObj.getSelectedSpecies().size()>0)){
					...
					if(aquaMapObj.getType().equals(ObjectType.Biodiversity)){
						t=new BiodiversityThread(waitingGroup,jobId,aquaMapObj.getId(),aquaMapObj.getName(),aquaMapObj.getThreshold(),actualScope);						
						((BiodiversityThread)t).setRelatedSpeciesList(aquaMapObj.getSelectedSpecies());
						...
						((DistributionThread)t).setRelatedSpeciesId(aquaMapObj.getSelectedSpecies());
